...
export const Route = createFileRoute("/admin/receipts/en/$paymentId")({ component: ReceiptPage });

function ReceiptPage() {
  const { paymentId } = useParams({ from: "/admin/receipts/en/$paymentId" });
  const fn = useServerFn(getPaymentReceipt);
  const { data, isLoading } = useQuery({
    queryKey: ["receipt-en", paymentId],
    ...
    <div className="min-h-screen bg-muted/40 py-6 print:bg-white print:py-0">
      <ReceiptShell autoOpen={!cfg.autoPrint}>
        ...
        >
          ...
          {row("Method", p.payment_method ?? p.method ?? "bKash")}
